...
class User
{
    ...
    function forceLogin()
    {
        if (isset($_SESSION['user_id']) && isset($_SESSION['authcode']) && isset($_SESSION['is_verified'])) {
            if ($_SESSION['is_verified']  == 1) {
                return TRUE;
            } else {
                header("Location: login/verify-account.php");
                exit;
            }
        } else {
    ...
    function findUser($un, $fetch_assoc = false)
    {
        ...
        $query = "SELECT `id`,`password`, `email` FROM `user` WHERE `username` = '" . $un . "' LIMIT 1";
        $result = $db->readQuery($query);
    ...
    public function SelectForgetUser($email)
    {
        ...
        if ($email) {
            ...
            $query = "SELECT `email`,`username`,`reset` FROM `user` WHERE `email`= '" . $email . "'";
    ...
    public function checkResetCode($id, $code)
    {
        ...
        $query = "SELECT `id` FROM `user` WHERE `reset`= '" . $code . "' AND `id` = $id";
